...
else{this.getCmp().setValue(this.hiddenField.getValue());}},destroy:function(){this.callParent(arguments);if(this.hiddenField){this.hiddenField.destroy();}},renderHiddenField:function(){var field=this.getCmp();if(field.ownerCt){field.ownerCt.items.add(this.hiddenField);}else{this.hiddenField.render(field.el.parent());}
field.inputEl.on("keydown",this.onKeyDown,this);},onDelete:function(caret,delta){var value=this.hiddenField.getValue(),split=caret;if(Ext.isNumber(caret)&&(this.getCaretPosition()<caret)){split=caret-delta;}
...
field.setValue(tmp);if(field.hasFocus){this.restoreCaretPos(caret);}}},onKeyDown:function(e){var me=this,oldValue=me.cmp.lastValue;if(me.mode!="hideall"){return;}
setTimeout(function(){var newValue=me.cmp.getValue(),key=e.getKey();if((newValue.length<oldValue.length)&&((key===e.BACKSPACE)||(key===e.DELETE))){me.onDelete(me.getCaretRange(),oldValue.length-newValue.length);}else{me.maskChars(newValue);}
