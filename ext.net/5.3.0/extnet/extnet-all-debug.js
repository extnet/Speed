...
Ext.apply(Ext.dom.Element, {
    serializeForm : function (form, parentEl) {
        var data = [],
            ...
            element, hasValue, name, options, type;
...
Ext.define("Ext.net.ClickRepeater", {
    ...
    // Checks whether the event-bound button (left, right or middle) should be
    // ignored.
    ...
    privates: {
        fireClick: function (e) {
            ...
            // Only fire the click event if the button is not ignored
            if (!me.isButtonIgnored(e)) {
                // After the click event is successfully fired, call
                // Ext.NET-specific left/right/middle button click events
                if (me.fireEvent("click", me, e) !== false) {
                    if (me.preventDefault) e.preventDefault();
                    if (me.stopDefault) e.stopEvent();

                    if (me.btnEvents[e.button]) {
                        me.fireEvent(me.btnEvents[e.button], me, e);
                    }
                }
        ...
        // Adds a contextmenu handler so that it is prevented should either
        // preventDefault or stopDefault ClickRepeater options be set.
        updateEl: function (newEl, oldEl) {
            var me = this,
                retVal = me.callParent(arguments),
                elListeners;

            if (newEl && (me.preventDefault || me.stopDefault)) {
                if (!me.elListeners["contextmenu"]) {
                    // Inject the contextmenu listener to the list of active
                    // listeners so that it is properly wiped on destroy()
                    me.elListeners.args[0].contextmenu = me.eventOptions;

                    newEl.on("contextmenu", me.eventOptions, me);
                }
            }
...
Ext.define("extnet-ext-component", {
...
// This fixes an issue where hiding/destroying a component will put z-Index manager
// in a state where there's no component 'in front/focused'.
Ext.define("gh1550", {
    override: "Ext.Component",
    hide: function () {
        var me = this,
        ...
        // Sets the topmost component to the 'front' one if the front component was
        // hid and there's no component on front anymore.
        if (selectTopMostCmp && zim.front === undefined) {
            zim.front = zim.topMost;
        }
...
Ext.ComponentLoader.Renderer.Data = function (loader, response, active) {
    var success = true,
        componentCode = response.responseText;

    try {
        loader.getTarget().update((Ext.isObject(componentCode) || Ext.isArray(componentCode)) ? componentCode : eval("(" + componentCode + ")"));
    } catch (e) {
        success = false;
    }
...
Ext.ComponentLoader.Renderer.Component = function (loader, response, active) {
    ...
    try {
        items = (Ext.isObject(componentCode) || Ext.isArray(componentCode)) ? componentCode : eval("(" + componentCode + ")");
    } catch (e) {
        success = false;
    }
...
Ext.ComponentLoader.Renderer.Script = function (loader, response, active) {
    ...
    try {
        if (window.execScript) {
            window.execScript(componentCode);
        } else {
            window.eval.call(window, componentCode);
        }
    } catch (e) {
        success = false;
    }
    ...
    loadFrame: function (options) {
        ...
        if (Ext.isEmpty(target.iframe)) {
            ...
            if (Ext.isIE && Ext.ieVersion <= 9) {
                // IE9 refresh iframe with pdf issue: https://forums.ext.net/showthread.php?24690
...
Ext.define("Ext.net.ImageButton", {
    ...
    initComponent: function () {
        ...
        if (me.imageUrl) {
            i = new Image().src = me.imageUrl;
        }

        if (me.overImageUrl) {
            i = new Image().src = me.overImageUrl;
        }

        if (me.disabledImageUrl) {
            i = new Image().src = me.disabledImageUrl;
        }

        if (me.pressedImageUrl) {
            i = new Image().src = me.pressedImageUrl;
        }
    },
    ...
    getElConfig: function () {
        ...
        Ext.applyIf(config.style, me.getStyle());
    ...
    onRender: function (ct, position) {
        ...
        if (!Ext.isEmpty(me.imgEl.getAttribute("width"), false) ||
            !Ext.isEmpty(me.imgEl.getAttribute("height"), false)) {
            me.imgEl.dom.removeAttribute("width");
            me.imgEl.dom.removeAttribute("height");
        }

        if (me.altText) {
            me.imgEl.dom.setAttribute("alt", me.altText);
        }

        if (me.align && me.align !== "notset") {
            me.imgEl.dom.setAttribute("align", me.align);
        }

        if (me.pressed && me.pressedImageUrl) {
            me.imgEl.dom.src = me.pressedImageUrl;
        }

        if (me.tabIndex !== undefined) {
            me.imgEl.dom.tabIndex = me.tabIndex;
        }

        //me.imgEl.on(me.clickEvent, me.onClick, me);
    ...
    onMenuHide: function (e) {
        ...
        me.imgEl.dom.src = (me.monitoringMouseOver) ? me.overImageUrl : me.imageUrl;
    ...
    toggle: function (state, suppressEvent, suppressHandler) {
        ...
        if (state != me.pressed) {
            ...
            if (!suppressEvent) {
                me.fireEvent("toggle", me, me.pressed);
            }

            if (me.toggleHandler && !suppressHandler) {
                me.toggleHandler.call(me.scope || me, me, state);
            }
        }
        return me;
    },
    ...
    setDisabled: function (disabled) {
        ...
        if (me.imgEl && me.imgEl.dom) {
            me.imgEl.dom.disabled = disabled;
        }

        if (disabled) {
            if (me.disabledImageUrl) {
                me.imgEl.dom.src = me.disabledImageUrl;
            } else {
                me.imgEl.addCls(me.disabledCls);
            }

            me.imgEl.setStyle({ cursor: "default" });
        } else {
            me.imgEl.dom.src = me.imageUrl;
            me.imgEl.setStyle({ cursor: "pointer" });
            me.imgEl.removeCls(me.disabledCls);
        }
    },
    ...
    onMouseOver: function (e) {
        ...
        if (!me.disabled) {
            var internal = e.within(me.el.dom, true);

            if (!internal) {
                if (me.overImageUrl && !me.pressed && !me.hasVisibleMenu()) {
                    me.imgEl.dom.src = me.overImageUrl;
                }

                if (!me.monitoringMouseOver) {
                    Ext.getDoc().on("mouseover", me.monitorMouseOver, me);
    ...
    monitorMouseOver: function (e) {
        ...
        if (e.target != me.el.dom && !e.within(me.el)) {
            if (me.monitoringMouseOver) {
                Ext.getDoc().un('mouseover', me.monitorMouseOver, me);
    ...
    onMouseEnter: function (e) {
        ...
        if (me.overImageUrl && !me.pressed && !me.disabled && !me.hasVisibleMenu()) {
            me.imgEl.dom.src = me.overImageUrl;
        }
        me.fireEvent("mouseover", me, e);
    },
    ...
    onMouseOut: function (e) {
        ...
        if (!me.disabled && !me.pressed && !me.hasVisibleMenu()) {
            me.imgEl.dom.src = me.imageUrl;
        }
        ...
        if (Ext.isIE || e.pointerType === 'touch') {
            ...
            Ext.defer(function () {
                ...
                if (!e.defaultPrevented) {
                    ...
                    // The component might be destroyed by me time
                    if (focusEl) {
                        focusEl.focus();
                    }
                }
    ...
    onMouseUp: function (e) {
        var me = this;
        if (!me.destroyed && e.button === 0 && !me.enableToggle && !me.hasVisibleMenu()) {
            me.imgEl.dom.src = (me.overImageUrl && me.monitoringMouseOver) ? me.overImageUrl : me.imageUrl;
        }
    },
    ...
    setImageUrl: function (image) {
        ...
        if ((!me.disabled || Ext.isEmpty(me.disabledImageUrl, false)) &&
            (!me.pressed || Ext.isEmpty(me.pressedImageUrl, false))) {
            me.imgEl.dom.src = image;
        } else {
            new Image().src = image;
        }
    },
    ...
    setPressedImageUrl: function (image) {
        ...
        me.pressedImageUrl = image;

        if ((!me.disabled || Ext.isEmpty(me.disabledImageUrl, false)) && me.pressed) {
            me.imgEl.dom.src = image;
        } else {
            new Image().src = image;
        }
    ...
    setAltText: function (altText) {
        ...
        if (me.rendered) {
            me.imgEl.dom.setAttribute("altText", me.altText);
        }
    }
});
...
Ext.form.field.Base.override({
    ...
    remoteValidate: function () {
        ...
        me.rvConfig.remoteValid = false;
        me.rvConfig.remoteValidated = false;

        var dc = Ext.apply({}, me.remoteValidationOptions),
            options = { params: {} };

        if (me.fireEvent("beforeremotevalidation", me, options) !== false) {
            dc.userSuccess = me.remoteValidationSuccess;
            dc.userFailure = me.remoteValidationFailure;
            ...
            if (me.rvConfig.showBusy) {
                me.saveIndicator("remotevalidation", true);
                me.setIndicatorIconCls(me.rvConfig.busyIconCls, true);
                me.showIndicator();

                if (me.rvConfig.busyTip) {
                    me.setIndicatorTip(me.rvConfig.busyTip);
                }
            }

            me.rvConfig.remoteValidated = false;
            me.rvConfig.validating = true;
            me.rvConfig.lastValue = o.value;

            me.wasValid = false;
            me.fireEvent('validitychange', me, false);

            if (me.validationId) {
                me.validationId.abortedByEvent = true;

                try {
                    Ext.net.DirectEvent.abort(me.validationId);
                } catch (e) { }
            }
            ...
            if (directFn) {
                ...
                if (directFn.length === 2) {
                    me.validationId = directFn(o.value, dc);
                }
                else if (directFn.length === 3) {
                    me.validationId = directFn(o.value, o.name, dc);
                }
                else if (directFn.length === 4) {
                    me.validationId = directFn(o.value, o.name, o.id);
                }
                else {
                    me.validationId = directFn(o.value, o.name, o.id, extraParams || null, dc);
                }
            }
            else {
                me.validationId = Ext.net.DirectEvent.request(dc);
            }
        }
    },
...
Ext.net.FieldNote = {
    ...
    // GitHub issue #1621: calls updateLayout() when changes affects the field's
    // used width/height in the hideNote(), showNote() and setNote() methods.

    hideNote: function () {
        var me = this;

        if (!Ext.isEmpty(me.note, false) && me.noteEl) {
            me.noteEl.setStyle("display", "none");
            me.updateLayout();
        }
    },
    ...
    updateIndicator: function () {
        ...
        parentIsIndicator = parentToIndicator.id !== undefined && Ext.net.StringUtils.endsWith(parentToIndicator.id || "", '-indicator');

        hideEl = parentIsIndicator ? this.indicatorEl.parent() : this.indicatorEl;
        ...
        if (errorSide) {
            ...
            hideEl.setStyle("display", (hideIndicator && !this.preserveIndicatorIcon) ? "none" : "table-cell");
...
Ext.define("extnet-ext-form-field-checkbox", {
...
// Improved indicator + field notes support for checkbox/radio components
Ext.define('gh1417', {
    override: 'Ext.form.field.Checkbox',

    // Special treatment for the indicator if the field has also a note.
    // This allows the note to span beyond the checkbox width, yet not
    // pull the indicator character away from the checkbox.
    updateIndicator: function () {
        var me = this,
            displayEl = me.displayEl,
            el = me.el,
            indicatorEl = me.indicatorEl,
            innerWrapEl = me.innerWrapEl,
            labelEl = me.labelEl,
            noteEl = me.noteEl,
            noteEnabled = noteEl && displayEl,
            isTopNote = this.noteAlign == "top",
            fieldHeight, retVal;

        retVal = this.callParent(arguments)

        if (!me.indicatorMovedToWrapEl) {
            // move the indicator element inside the innerWrapEl.
            // this will allow the note to be wide without pulling away the
            // element containing the indicator
            innerWrapEl.appendChild(indicatorEl);

            // Fix position of the indicator within innerWrapEl
            indicatorEl.setX(me.labelPad + displayEl.getX() + displayEl.getWidth());

            me.indicatorMovedToWrapEl = true;

            if (noteEnabled) {
                fieldHeight = parseInt(displayEl.getStyle("margin-top").replace(/px$/, ''));
                if (isTopNote) {
                    fieldHeight += noteEl.getHeight();

                    if (me.fieldLabel) {
                        labelEl.setStyle("padding-top", fieldHeight + "px");
                    }
                } else {
                    fieldHeight += displayEl.getHeight();
                    noteEl.setStyle("margin-top", fieldHeight + "px");
                }
            }
        }

        // This is kept being reset if note is top, then we need to keep setting this back
        if (noteEnabled && isTopNote) {
            indicatorEl.setStyle('top', 0);

            if (!me.fieldLabel) {
                // enforce minimum height
                fieldHeight = parseInt(displayEl.getStyle("margin-top").replace(/px$/, '')) +
                    noteEl.getHeight() + displayEl.getHeight();

                // only fix height if it was not force-set and above minimum
                if (!(Ext.isNumber(me.height) && me.height >= fieldHeight)) {
                    el.setHeight(fieldHeight);
                }
            }
        }

        // this return just guarantees if ever the parent returns anything
        // the caller will get what it expected.
        return retVal;
    }
});

// For checkboxes there's not much to validade (it can only be true or false),
// and also its position is fixed, so there shouldn't be layout issues with
// this approach to hide/show the indicator icon. This is specially necessary
// due to the fix 'gh1417' above.
Ext.define('gh1569', {
    override: 'Ext.form.field.Checkbox',
    showIndicator: function () {
        ...
        me.indicatorEl.show();
        me.indicatorHidden = false;
    },
    hideIndicator: function () {
        var me = this;
        me.indicatorEl.hide();
        me.indicatorHidden = true;
    }
});

// Drops the innerWrapEl element from the 'display' CSS property when showing
// a previously hidden checkbox field.
// This most likely is a side effect of the modifications above that allow
// better indicator and field notes' interaction on checkboxes.
Ext.define("gh1567", {
    override: "Ext.form.field.Checkbox",
...
Ext.define('gh1598', {
    ...
    assertValue: function () {
        ...
        // This changed the value. We should refresh previous valid state.
        if (me.rawValue != prevVal) {
            me.wasValid = me.isValid();
        }

        // return whatever the original returned -- even if that's 'undefined'
        return retVal;
    }
});

// Fix here as suggested at:
// https://www.sencha.com/forum/showthread.php?299507
Ext.define("gh1683", {
    override: "Ext.form.field.ComboBox",
...
Ext.define("Ext.net.DropDownField", {
    ...
    // code adapted from Ext.form.field.Field.reset() as of ExtJS 6.5.0.
    reset: function () {
        var me = this;

        me.beforeReset();
        me.setValue(me.originalValue, me.originalText, false);
        me.clearInvalid();
        delete me.wasValid;
    },
...
Ext.data.Model.override({
    inheritableStatics: {
        replaceFields: function (newFields, removeFields) {
            ...
            // Ext.NET-specific code (begin)
            // introduced on commit 789621c397f065627489db55a5c192eafbc82cce
...
Ext.define('gh1610', {
    ...
    doRequest: function () {
        ...
        // If the data proxy defines responseType and it is 'json',
        // tamper the definition removing it if we are using Ext JS
        // implementation of JSON handlers.
        if (Ext.isDefined(me.responseType)) {
            orgrt = me.responseType;
            if (orgrt == 'json' && !Ext.USE_NATIVE_JSON) {
                delete me.responseType;
            };
        }

        retVal = me.callParent(arguments);

        // Restore setting after making the request -- if it was overridden.
        if (Ext.isDefined(orgrt) && orgrt == 'json' && !Ext.USE_NATIVE_JSON) {
            me.responseType = orgrt;
        }
...
// @source data/request/Form.js

// Issues below are closely tied so they had to be merged in
// this single override.
// Issue 1527 clones the form before proceeding to callParent()
// Issue 1535 cycles thru the radio elements fixing them
// Issue 1552 handles file input fields moving for failed cloneNode()
Ext.define('gh1527_1535_1552', {
    override: 'Ext.data.request.Form',
    upload: function (form, url, params, options) {
        var me = this,
            extnetCall = false,
            movedFields = [],
            el, i, moved, oldNode, origForm, retVal
        hasFileAPI = !(Ext.isIE && Ext.ieVersion < 10);
        ...
        if (!hasFileAPI) {
            if (Ext.isDebugEnabled()) {
                Ext.raise("Ext.data.request.Form.upload(): Current browser is known not to support the File API for file input fields: " + Ext.browser.name + " v" + Ext.browser.version.toString());
            } else {
                Ext.Logger.warn("Ext.data.request.Form.upload(): Current browser is known not to support the File API for file input fields: " + Ext.browser.name + " v" + Ext.browser.version.toString())
            }
        }
        ...
        if (extnetCall) {
            ...
            // Recursively clone the form for submission mangling.
            // ExtJS will eventually drop it.
            form = Ext.getDom(form).cloneNode(true);

            // Cleanup radio elements in the cloned form.
            if (form.elements) {
                // remove the 'name' attribute from any radio element, so that
                // the 'hidden' resulting value does not get overridden.
                for (i = 0; i < form.elements.length; i++) {
                    el = form.elements[i];
                    if (el.type == "radio") {
                        // also ensure it is an ExtJS inputEl radio before removing.
                        if (el.name !== undefined && Ext.net.StringUtils.endsWith(el.id || "", "-inputEl")) {
                            el.removeAttribute('name');
                        }
                    } else if (hasFileAPI && el.type === "file" && el.tagName === "INPUT" &&
                        el.files.length != origForm.dom.elements[i].files.length) {
                        // MS-IE and MS-Edge do not fully clone file input elements, so
                        // effectively the resulting "clone" won't have any attached files.
                        // Checking if the cloned input files' length differs is enough and
                        // robust approach to define whether it is necessary or not.

                        // Then we list the entries to be moved out and back in later
                        movedFields.push({
                            idx: i,
                            field: origForm.dom.elements[i],
                            parent: origForm.dom.elements[i].parentElement
                        });
                    }
                }
            }
        ...
        // If there's any element to be DOMwise moved around, do it, removing
        // its failed cloneNode() attempts.
        // Reverse index because once one is removed all following's indexes will
        // be pulled up.
        for (i = movedFields.length - 1; i >= 0; i--) {
            moved = movedFields[i];

            oldNode = form.elements[moved.idx];

            // Could call just oldNode.remove() but IE requires oldNode.removeNode(),
            // so use a way that's supported by all browsers. MS-Edge uses oldNode.remove().
            oldNode.parentElement.removeChild(oldNode);

            form.appendChild(moved.field);
        }

        retVal = me.callParent(arguments);

        // Add back any moved fields to their original parent element.
        if (movedFields.length > 0) {
            for (i = 0; i < movedFields.length; i++) {
                moved = movedFields[i];
                moved.parent.appendChild(moved.field);
            }
        }
    }
});
Ext.data.writer.Writer.override({
    ...
    getRecordData: function (record, operation) {
        var me = this,
            ...
            key = idField ? (idField[nameProperty] || idField.name) : 'id', // setup for idField first
            value = record.id,
        ...
        if (!writeAll && operation && operation.isDestroyOperation) {
            ...
            // On Ext.NET, we will always need it to be an object, even empty,
            // so that we can bind the 'serialize' setting to it.
            if (options === null) {
                options = {}
            };
            options.serialize = false; // Ext.NET, we must take over this here
            ...
            for (key in data) {
                ...
                // Ext.NET-specific for filtering fields.
                ...
                if (!(field = fieldsMap[key])) {
                    ...
                    // Ext.NET-specific if-else block
...
// @source: data/Query.js

// #1628: Stubs the 'getProperty()' method so that GridFilters can call it.
// The stub just always returns null/undefined (whatever Ext.emptyFn returns),
// meaning the affected field by the Query filter is always arbitrary or
// unspecified, i.e. depends on the query and can affect various fields.
Ext.define('gh1628', {
    override: 'Ext.data.Query',
...
// The override here emphasizes in retaining original code (by using CallParent)
// and fixing the issue -- this reduces the odds for changes in the original method
// to cause conflicts or require manual merging between Ext JS version upgrades.
Ext.define('gh1687', {
...
Ext.define('Ext.net.RowExpander', {
    ...
    toggleRow: function (rowIdx, record, state) {
        ...
        var me = this,
            ...
            addOrRemoveCls = (hasState && state) || wasCollapsed ? 'removeCls' : 'addCls',
            needsToExpand = wasCollapsed && (!hasState || state === true),
            needsToCollapse = !wasCollapsed && (!hasState || state === false),
            needsToToggle = needsToExpand || needsToCollapse,
            grid = this.grid,
            rowCmp,
            needContainer,
            ownerLockable = me.grid.ownerLockable,
        ...
        // needsToToggle means either needsToExpand or needsToCollapse being true, so if
        // then needsToExpand (below) is not true, then needsToCollapse is.
        if (needsToToggle) {
            fireView.fireEvent(needsToExpand ? 'expandbody' : 'collapsebody', rowNode, record, nextBd);
        }
...
Ext.define("Ext.grid.column.CommandColumn", {
    ...
    onItemMouseLeave: function (view, record, item, index, e) {
        ...
        if (me.showDelayTask) {
            Ext.undefer(me.showDelayTask);
            delete me.showDelayTask;
        }

        if (me.hideDelay) {
            if (me.hideDelayTask) {
                Ext.undefer(me.hideDelayTask);
            }
    ...
    hideToolbar: function (view, record, item, index, e) {
        ...
        if (this.showDelayTask) {
            Ext.undefer(this.showDelayTask);
    ...
    showToolbar: function (view, record, item, index, e) {
        ...
        if (this.hideDelayTask) {
            Ext.undefer(this.hideDelayTask);
            delete this.hideDelayTask;
        }
...
// @source src/grid/Column/RowNumbererColumn.js
...
Ext.define('gh1317', {
    ...
    defaultRenderer: function (value, metaData, record, rowIdx, colIdx, dataSource, view) {
        ...
        /// Support for the grouping grid panel feature.
        if (page === undefined && dataSource.groupingFeature && dataSource.store &&
            dataSource.store.currentPage && Ext.isNumber(dataSource.store.currentPage) &&
            dataSource.store.pageSize && Ext.isNumber(dataSource.store.pageSize)) {
            page = dataSource.store.currentPage;
            pageSize = dataSource.store.pageSize;
        }
...
Ext.grid.selection.SpreadsheetModel.override({
    ...
    deselect: function (records, suppressEvent) {
        ...
        // The 'records' here will override also within arguments[0]
        records = me.parseRows(records); // Defined in Ext.NET for better API
...
Ext.define("Ext.net.MenuPanel", {
    ...
    // This may be an array of strings or a string containing the CSS
    // class(es) to apply to the menu item once it is selected.
    // By default, borrows the selected row color effect from the
    // GridPanel. Should work fine on most themes as this is intended
    // for a very similar situation on the borrowing component.
    selectedCls: 'x-grid-item-selected',

    // analog to above, but applies to the text span on the component, for
    // for text-related effects
    selectedTextCls: undefined,
    ...
    initComponent: function () {
        ...
        // move the menu to the items collection so that the
        // upper initComponent iterate thru it.
        me.items = [me.menu];

        me.callParent(arguments);

        // copy back the built menu.
        me.menu = me.items.get(0);

        if (me.selectedIndex > -1) {
            var item = me.menu.items.get(me.selectedIndex);

            if (item.rendered) {
                me.setItemHighlight(item, true);
            } else {
                item.on("afterrender", function () { me.setItemHighlight(item, true); }, me, { single: true });
            }

            me.getSelIndexField().setValue(me.selectedIndex);
        }

        me.menu.on("click", me.setSelection, me);
    },

    setSelectedIndex: function (index) {
        var me = this;

        me.setSelection(me.menu, me.menu.getComponent(index));
    },
    ...
    getSelIndexField: function () {
        ...
        if (!me.selIndexField) {
            me.selIndexField = new Ext.form.Hidden({ id: me.id + "_SelIndex", name: me.id + "_SelIndex" });

            me.on("beforedestroy", function () {
                me.destroy();
            }, me.selIndexField, { single: true });
        }

        return me.selIndexField;
    },

    setItemHighlight: function (item, enable) {
        var me = this,
            clsFn, styleFn, clsTFn, styleTFn;

        if (enable === true) {
            clsFn = item.addCls;
            clsTFn = item.textEl.addCls;
        } else {
            clsFn = item.removeCls;
            clsTFn = item.textEl.removeCls;
        }

        if (me.selectedCls !== undefined) {
            clsFn.apply(item, [me.selectedCls]);
        }

        if (me.selectedTextCls !== undefined) {
            clsTFn.apply(item.textEl, [me.selectedTextCls]);
        }
    },
    ...
    setSelection: function (menu, item, e) {
        ...
        if (me.saveSelection) {
            ...
            if (item) {
                me.clearSelection();
                me.selectedIndex = me.menu.items.indexOf(item);
                me.getSelIndexField().setValue(me.selectedIndex);
                me.setItemHighlight(item, true);
            }
        }
    ...
    clearSelection: function () {
        ...
        if (me.selectedIndex > -1) {
            selectedCmp = me.menu.getComponent(me.selectedIndex);

            if (selectedCmp) {
                me.setItemHighlight(selectedCmp, false);
            }
        }

        me.selectedIndex = -1;
        me.getSelIndexField().setValue(null);
    },
    ...
    afterRender: function () {
        ...
        if (me.hasId()) {
            me.getSelIndexField().render(me.el.parent() || me.el);
        }
    }
});
...
Ext.menu.ColorPicker.override({
    // This has been overridden because cfg ignores me.pickerConfig in original
    // implementation
    // This override has no github issue history, so probably it implements the
    // Ext.NET-specific feature so consider its pickerConfig at config-time
    // (<Picker runtat="server" Draggable="true" ... />)
    initComponent: function () {
        var me = this,
            initialConfig = me.initialConfig;

        // adds pickerConfig settings straight to initialConfig, preserving
        // initialConfig data that do not overlap.
        me.initialConfig = Ext.apply({}, me.pickerConfig, me.initialConfig);

        var retVal = me.callParent(arguments);

        // restore initial config, just in case.
        me.initialConfig = initialConfig;

        // we don't really need to worry, just for robustness, return whatever
        // callParent returns, if it does.
        return retVal;
    }
});
...
Ext.define('gh1597', {
    ...
    setUI: function (ui) {
        ...
        // Pass 'primary' as parameter if nothing meaningful specified
        retVal = me.callParent((ui === undefined || ui.length < 1) ? ['primary'] : arguments);

        // If the effective UI changed after calling Ext.Component's setUI(),
        // then reflect the change to the progress bar's 'bar' subcomponent.
        if (me.bar && curUI != me.activeUI) {
            me.bar.removeCls(me.baseCls + '-bar-' + curUI);
            if (me.activeUI && me.activeUI.length && me.activeUI.length > 0) {
                me.bar.addCls(me.baseCls + '-bar-' + me.activeUI);
            }
        }
...
Ext.define('gh1585', {
    ...
    onSelectChange: function (record, isSelected, suppressEvent, commitFn) {
        ...
        // Return at once if can't fetch the current position.
        if (pos == null) return;
...
// @source selection/CheckboxModel.js
Ext.define("Ext.selection.CheckboxModel", {
    ...
    renderer: function (value, metaData, record, rowIndex, colIndex, store, view) {
        var me = this,
            selMdl, checkCol, columnClassProto;

        // #1633: 'me' can be the column itself, not the CheckboxModel
        // instance, when this method is called when bound to the column.
        if (me.column !== undefined) {
            selMdl = me;
            checkCol = me.column;
        } else {
            selMdl = me.getView().getSelectionModel();
            checkCol = me;
        }

        columnClassProto = Ext.ClassManager.classes[checkCol.$className].prototype;
...
// Ideally, this fix should have been in a specific theme-material.js override, but currently
// Ext.NET does not implement a way to apply theme-specific overrides.
Ext.define('gh1686', {
    ...
    initComponent: function () {
        ...
        // This test is only required if this code is not in ext-material.js
...
Ext.tab.Bar.override({
    ...
    addTab: function (card, index, activate) {
        var me = this,
            config = {},
            tabIsHidden = false,
            existingCard, tab;
        ...
        if (card.id) {
            existingCard = me.tabPanel.getComponent(card.id);

            if (existingCard && existingCard.tab && existingCard.tab.hidden) {
                tabIsHidden = true;
            }
        }

        if (tabIsHidden) {
            if (existingCard.hidden && Ext.isFunction(existingCard.show)) {
                existingCard.show();
            }

            tab = existingCard.tab;

            if (!Ext.isEmpty(config.index) && config.index >= 0) {
                me.move(me.items.indexOf(tab), config.index);
            }

            tab.show();
        } else {
            if (card.hidden && Ext.isFunction(card.show)) {
                card.show();
            }

            if (!Ext.isEmpty(config.index) && config.index >= 0) {
                tab = me.tabPanel.insert(config.index, card).tab;
            } else {
                tab = me.tabPanel.add(card).tab;
            }
        }

        if (config.activate !== false) {
            me.tabPanel.setActiveTab(tab.card);
        }
    },
    ...
    closeTab: function (toClose) {
        ...
        if (toClose.$className === "Ext.tab.Tab") {
            tab = toClose;
            card = tab.card;
        } else {
            tab = toClose.tab;
        ...
        if (card && card.closeAction && card.closeAction === "hide") {
...
Ext.tab.Panel.override({
    ...
    closeTab: function () {
        return this.tabBar.closeTab.apply(this.tabBar, arguments);
    },

    
    addTab: function () {
        return this.tabBar.addTab.apply(this.tabBar, arguments);
    },
...
// Fixes issue with viewport not following browser window's resizes, introduced
// by Ext JS 6.7.0 (6.7.0.161)
Ext.define("gh1627", {
...
// Enables Ext.NET to bind the component to App.* (or namespace
// variable) if an ID was initially set to the component.
Ext.define('Ext.net.Widget', {
    override: 'Ext.Widget',
    constructor: function () {
        var me = this,
            retVal = me.callParent(arguments);

        Ext.net.ComponentManager.registerId(me);
    ...
    doDestroy: function () {
        ...
        Ext.net.ComponentManager.unregisterId(me);
