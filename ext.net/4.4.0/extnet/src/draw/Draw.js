...
Ext.draw.sprite.Sprite.override({
    setAttributes: function (changes, bypassNormalization, avoidCopy) {
        ...
        if (changes && me.modifiers && me.modifiers.animation) {
            fx = me.modifiers.animation;
            duration = Ext.isDefined(changes.duration),
            easing = Ext.isDefined(changes.easing);

            if (duration) {
                fx.setDuration(changes.duration);
            }

            if (easing) {
                fx.setEasing(changes.easing);
            }
        }
